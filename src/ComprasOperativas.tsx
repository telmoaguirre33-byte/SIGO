            ...
            <tbody>
              {lineas.map((l) => (
                <tr key={l.key}>
                  ...
                  <td><input type="number" min="1" step="1" value={l.cantidad || ""} onFocus={(e) => e.currentTarget.select()} onChange={(e) => editarLinea(l.key, { cantidad: e.target.value === "" ? 0 : Math.max(0, Math.trunc(Number(e.target.value))) })} /></td>
                  ...
                  <td><input type="number" step="0.01" value={l.margen_porcentaje || ""} onFocus={(e) => e.currentTarget.select()} onChange={(e) => { const margen=e.target.value===""?0:Number(e.target.value); editarLinea(l.key,{margen_porcentaje:margen,precio_venta:Math.round(l.costo_unitario*(1+margen/100)*100)/100}); }} /></td><td><input type="number" min="0" step="0.01" value={l.precio_venta || ""} onFocus={(e) => e.currentTarget.select()} onChange={(e) => { const precio=e.target.value===""?0:Number(e.target.value); editarLinea(l.key,{precio_venta:precio,margen_porcentaje:l.costo_unitario>0?((precio-l.costo_unitario)/l.costo_unitario)*100:0}); }} /></td><td>$ {(l.cantidad * l.costo_unitario).toLocaleString("es-AR", { minimumFractionDigits: 2 })}</td>
