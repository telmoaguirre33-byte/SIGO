...
async function soapWsfe(endpoint, action, innerXml) {
...
async function ultimoAutorizado(endpoint, ticket, cuit, puntoVenta, tipoCbte) {
  const { body, errors } = await soapWsfe(
    endpoint,
...
async function consultarComprobante(endpoint, ticket, cuit, puntoVenta, tipoCbte, numeroCbte) {
  const { body, errors } = await soapWsfe(
    endpoint,
    "FECompConsultar",
...
function detalleFiscal(tipoCbte, total, items, productMap) {
  if (tipoCbte === 11) {
    return { impTotal: total, impNeto: total, impIva: 0, ivaXml: "" };
  }
  ...
  for (const item of items) {
    ...
    if (!id || !Number.isFinite(tasa) || tasa < 0 || !Number.isFinite(bruto) || bruto <= 0) {
      throw new Error("ARCA_PRODUCT_FISCAL_DATA_REQUIRED");
    }
  ...
  if (alicuotas.length > 0) {
    const ivaAgrupado = dinero(alicuotas.reduce((sum, row) => sum + row.importe, 0));
    alicuotas[alicuotas.length - 1].importe = dinero(alicuotas[alicuotas.length - 1].importe + impIva - ivaAgrupado);
  }
...
async function solicitarCae(endpoint, ticket, cuit, request) {
  ...
  const { body, errors } = await soapWsfe(endpoint, "FECAESolicitar", `${authXml(ticket, cuit)}${detail}`);
  if (errors.length) throw wsfeError(errors);
...
export { detalleFiscal, solicitarCae, consultarComprobante, ultimoAutorizado };
...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!WSAA[config.ambiente] || !WSFE[config.ambiente]) return json(res, 409, { error: "ARCA_ENVIRONMENT_INVALID" });
    ...
    if (confirmation !== expected) {
      return json(res, 409, { error: config.ambiente === "produccion" ? "ARCA_PRODUCTION_CONFIRMATION_REQUIRED" : "ARCA_HOMOLOGATION_CONFIRMATION_REQUIRED" });
    }
    if (!/^\d{11}$/.test(String(config.cuit_emisor || "")) || config.wsaa_service !== "wsfe" || config.wsfe_version !== "WSFEv1") {
      return json(res, 409, { error: "ARCA_CONFIG_INVALID" });
    }
    if (!config.certificado_ref || !config.certificado_vence || Date.parse(config.certificado_vence) <= Date.now()) {
      return json(res, 409, { error: "ARCA_CERTIFICATE_INVALID" });
    }
    ...
    if (tipoCbte === 1 && (!venta.cliente_id || receptor.docTipo !== 80 || receptor.docNro.length !== 11 || receptor.condicionIvaReceptorId !== 1)) {
      return json(res, 409, { error: "ARCA_INVOICE_A_CLIENT_REQUIRED" });
    }
    ...
    if (reserva && (Number(reserva.punto_venta) !== puntoVenta || Number(reserva.tipo_cbte) !== tipoCbte || reserva.request_id !== requestId)) {
    ...
    if (reserva) {
      ...
      const ultimo = await ultimoAutorizado(WSFE[config.ambiente], ticket, config.cuit_emisor, puntoVenta, tipoCbte);
    ...
    if (reserva && observations.length) {
      try {
        ...
        // Conservar el error fiscal original aunque no se pueda guardar el diagnóstico.
      }
    }
    console.error("SIGO ARCA CAE", error?.code || error?.message || "UNKNOWN");
    if (error?.code === "WSFE_REJECTED" || error?.code === "WSFE_CAE_REJECTED") {
      return json(res, 422, { error: "ARCA_CAE_REJECTED", observaciones: observations });
    }
    if (/abort|timeout/i.test(String(error?.message || ""))) return json(res, 504, { error: "ARCA_TIMEOUT", retryable: true });
    return json(res, 502, { error: "ARCA_CAE_FAILED", retryable: Boolean(reserva) });
  }
}
