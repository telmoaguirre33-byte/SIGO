-- SIGO: corrige la confirmación de ventas sin cliente (efectivo, tarjeta, transferencia, etc.).
-- Causa real reproducida en producción: v_cliente era RECORD sin asignar y se accedía a
-- sus campos al evaluar el límite de crédito, abortando toda la transacción antes de
-- consolidar stock y caja. La corrección usa escalares inicializados y sólo evalúa crédito
-- dentro del circuito cuenta_corriente.
...
begin
  ...
  -- Valida forma antes de tocar stock y construye una huella estable del payload.
  ...
  if p_cliente_id is not null then
    ...
    select coalesce(saldo_actual, 0), limite_credito
      into v_cliente_saldo, v_cliente_limite
  ...
  loop
    ...
    if v_producto.precio_venta is null or v_producto.precio_venta <= 0 then raise exception 'PRODUCT_PRICE_REQUIRED: %', v_producto.nombre; end if;
    ...
    if not found then raise exception 'INSUFFICIENT_STOCK: %', v_producto.nombre; end if;
    v_total := v_total + round(v_producto.precio_venta * v_cantidad, 2);
  end loop;

  -- IMPORTANTE: nunca referenciar datos de cliente para efectivo/tarjeta/transferencia.
  if p_medio_pago = 'cuenta_corriente' then
    if v_cliente_limite is not null
       and coalesce(v_cliente_saldo, 0) + v_total > v_cliente_limite then
  ...
  if not found then raise exception 'SALE_HEADER_NOT_FOUND'; end if;
...
comment on function public.confirmar_venta_sigo_v2(uuid, jsonb, text, text, uuid) is
  'SIGO: venta atómica tenant-safe. Corrige ventas sin cliente; efectivo y demás medios descuentan stock y generan caja sin evaluar un RECORD de cliente no asignado.';
