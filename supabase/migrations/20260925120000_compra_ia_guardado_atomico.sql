-- Compra IA: one transaction for supplier/product creation + purchase + stock.
-- Does not alter existing purchase/product functions or existing business rows.
create table public.compra_ia_confirmaciones (
...
alter table public.compra_ia_confirmaciones enable row level security;
revoke all on public.compra_ia_confirmaciones from public, anon, authenticated;

create function public.guardar_compra_ia_sigo(
...
begin
  ...
  for v_item in select value from jsonb_array_elements(p_compra->'items') loop
    ...
    if v_producto_id is null then
      ...
      if not coalesce(public.tiene_permiso_empresa(p_empresa_id,'costs.read'),false) or not coalesce(public.tiene_permiso_empresa(p_empresa_id,'margins.read'),false)
        or not (coalesce(public.tiene_permiso_empresa(p_empresa_id,'price_lists.read'),false) or coalesce(public.tiene_permiso_empresa(p_empresa_id,'sales.write'),false)) then raise exception 'PRICE_PERMISSION_REQUIRED'; end if;
      ...
      if v_producto.precio_venta is distinct from v_precio then raise exception 'PRICE_PERMISSION_REQUIRED'; end if;
    end if;
